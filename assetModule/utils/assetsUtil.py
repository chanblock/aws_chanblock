...
def home():
    ...
    getvalues= list(client.chancblock.mtgoxblance.find().sort("t",1))
    dateX=[]
    valueY=[]
   
    for values in getvalues:
        dateX.append(values['t'])
...
def  graph(symbol):
    ...
    if type(symbol) != str:
        ...
        if len(symbol['metric']) == 1:            
            for i in symbol['assets']:
                getvalues= list(client.coinmetrics[i].find({symbol['metric'][0]:{"$exists":True}},{"_id":0, "time":1,symbol['metric'][0]:1}).sort("time",1))
                if len(getvalues)==0:
                    unmetric.append(i)
                else:    
                    listXY.append(getvalues)
        ...
        if len(symbol['metric']) > 1:  
                for i in symbol['metric']:
                    getvalues= list(client.coinmetrics[symbol['assets'][0]].find({i:{"$exists":True}},{"_id":0, "time":1,i:1}).sort("time",1))
                    if len(getvalues)==0:
                        unmetric.append(i)
                    else:  
                        listXY.append(getvalues)
    ...
    if len(listXY) != 0:
        ...
        if isArray:
            ...
            titleGraph='PriceUSD'
        print('******************',len(listXY),(Category20[20])[:len(listXY)])
        df= {'X_value':dataX,
            'Y_value':dataY,
            'model': modelSymbol,
            'color': (Category20[20])[:len(listXY)],
        ...
        pl.multi_line(xs='X_value', ys='Y_value', legend_group="model",
